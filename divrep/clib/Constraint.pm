package Constraint;
use strict;
use Math::Pari qw/ PARI lift Mod gcd divrem divisors factorint lcm /;
...
sub new {
	...
	my $self = bless {
		...
		mult => PARI(1),	# d = mod_mult (mod mult) is required by constraints
		mod_mult => PARI(0),
		tests => 0,
		skipped => 0,
		kept => 0,
	...
	$_ = PARI($_ || 0) for @$self{qw/ n f min max tau /};
	return $self;
}
...
sub c {
	my($self, $n) = @_;
	$self->{'c'}{$n} ||= do {
		my $div = _highfactors($n);	# x: x>1, px=n
		my $c = [
			$n,		# [0] the modulus
...
sub pack_sc {
	...
	$_->{'potency'} = $_->{'ncheck'} / $_->{'n'} for @aux;
 ...
 * cur: PARI integer, the current value to try
 * mult: PARI integer, the step to increment cur by
